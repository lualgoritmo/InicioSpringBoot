...
class InvoiceControllerTest {
    ...
    void getInvoices() throws Exception {
        // GIVEN:
        // CRIA O CARD HOLDER POIS ELE É A BASE DE TUDO E ENTÃO JÁ SALVA NO BANCO
        ...
        // Com cardHolder criado, cria a invoice apenas se houver transações
        ...
        if (!transactions.isEmpty()) {
            // Se houver transações, cria a invoice
            Invoice invoice =
                    new Invoice(
                            30.0f,
                            "IN_PROGRESS",
            ...
            // Faz o set dos card para dentro da transaction
            transactionList.forEach(transaction -> transaction.setCard(cardHolder.getCard()));

            // Seta qual é a invoice dessa transaction
            transactionList.forEach(transaction -> transaction.setInvoice(invoice));

            // Como no invoice o valor que vc passou (30) não cria a fatura com aquele valor,
            // aqui vc seta o valor certo da invoice
            invoice.setTotal(30.0f);
        ...
        result.andExpect(MockMvcResultMatchers.status().isOk())
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$.cardHolder.birthDate").value("01-10-1952"))
